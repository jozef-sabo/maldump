...
class McafeeExtractor(Extractor):
    ...
    def compute(self) -> QuarEntry:
        ...
        parser = self._get_data(file_name=self.quarentry.local_path)
        if parser is None:
            logger.debug('Skipping entry, path "%s"', self.quarentry)

        self.quarentry.malfile = parser["mal_file"]
